  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
        <div>
          <h2 className="text-2xl font-bold text-slate-900 tracking-tight">Test Management</h2>
          <p className="text-slate-500 text-sm font-medium">Create and manage student examinations and practice tests.</p>
        </div>
        <button
          onClick={() => setIsModalOpen(true)}
          className="flex items-center gap-2 px-5 py-2.5 bg-primary-600 hover:bg-primary-700 text-white rounded-xl font-bold text-[10px] uppercase tracking-widest transition-all shadow-lg shadow-primary-500/10 active:scale-95"
        >
      ...
      <div className="grid grid-cols-3 gap-6">
        {[
          ...
          return (
            <div key={stat.label} className="glass-dark border border-slate-100 rounded-2xl shadow-sm p-5 flex items-center gap-4 hover:shadow-md transition-all duration-300 group">
              <div className={`w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 border border-slate-100 ${stat.bg} group-hover:scale-110 transition-transform duration-500 shadow-sm`}>
      ...
      {/* Table Card */}
      <div className="glass-dark border border-slate-100 rounded-2xl shadow-sm overflow-hidden">
        {/* Toolbar */}
        <div className="p-5 border-b border-slate-100 bg-slate-50/30 flex flex-col sm:flex-row gap-4 justify-between">
          <div className="relative flex-1 max-w-sm">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
            <input
              type="text" value={search} onChange={e => setSearch(e.target.value)}
              placeholder="Search by title or category..."
              className="w-full pl-10 pr-5 py-2 bg-white border border-slate-200 rounded-xl text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-primary-500/20 transition-all placeholder:text-slate-400 shadow-sm"
            />
          </div>
        ...
        <div className="overflow-x-auto">
          {isLoading ? (
            <div className="flex flex-col items-center justify-center py-20 gap-4">
              <div className="w-10 h-10 rounded-full border-4 border-white/5 border-t-primary-500 animate-spin"></div>
              <p className="text-[10px] font-bold uppercase tracking-widest text-slate-500 animate-pulse">Syncing Test Core...</p>
            </div>
          ) : (
                  ...
                  <tr key={test.id} className="hover:bg-slate-50/50 transition-colors group">
                    <td className="px-6 py-5">
                      <div className="flex items-center gap-3">
                        ...
                        <p className="text-sm font-bold text-slate-900 group-hover:text-primary-600 transition-colors max-w-[180px] truncate" title={test.title}>{test.title}</p>
                      </div>
                    </td>
                    <td className="px-6 py-5">
                      <div className="flex items-center gap-2">
                        <BookOpen className="w-3.5 h-3.5 text-slate-400 flex-shrink-0" />
                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest max-w-[140px] truncate">{test.course}</span>
                      </div>
                    </td>
                    <td className="px-6 py-5">
                      {test.type === 'Practice' ? (
                        <div className="flex flex-col gap-1">
                          <span className="inline-flex w-fit items-center px-2 py-0.5 rounded-lg text-[10px] font-bold bg-primary-50 text-primary-600 uppercase tracking-widest border border-primary-100">Practice Mode</span>
                          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">{test.duration} limit</span>
                        </div>
                      ) : (
                        <div>
                          ...
                          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">{test.time} · {test.duration}</p>
                        </div>
                      )}
                    </td>
                    <td className="px-6 py-5 text-center">
                      <div className="flex items-center justify-center gap-2">
                        <span className="flex items-center gap-1.5 text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                          <BarChart2 className="w-3 h-3 text-primary-600" />{test.totalMarks}M
                        </span>
                        <span className="text-slate-200">|</span>
                        <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{test.questions} Qs</span>
                      </div>
                    </td>
                    <td className="px-6 py-5 text-center">
                    ...
                    <td className="px-6 py-5 text-center">
                      <span className={`inline-flex items-center gap-2 px-3 py-1 rounded-lg text-[10px] font-bold uppercase tracking-widest border ${
                        ...
                        {test.status === 'upcoming' ? <Clock className="w-2.5 h-2.5" /> : <CheckCircle2 className="w-2.5 h-2.5" />}
                        {test.status}
                      </span>
                    </td>
                      ...
                      <div className="flex items-center justify-end gap-1.5">
                        {test.status === 'upcoming' && !isScheduledTimePassed(test) && (
                          <button
                            ...
                            className="mr-1 px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white text-[10px] font-bold uppercase tracking-widest rounded-lg transition-all shadow-lg shadow-emerald-500/10 active:scale-95"
                          >
                            Launch
                          </button>
                        )}
                        <button
                          ...
                          className="px-2.5 py-1.5 bg-primary-50 hover:bg-primary-600 hover:text-white text-primary-600 text-[10px] font-bold uppercase tracking-widest border border-primary-100 rounded-lg transition-all active:scale-95 flex items-center gap-1.5"
                          title="View Results"
                        >
        ...
        {/* Footer */}
        <div className="px-6 py-4 border-t border-slate-100 text-[10px] font-bold uppercase tracking-widest text-slate-400 bg-slate-50/30 flex justify-between items-center">
