...
import { ArrowLeft, Save, Plus, Trash2, Edit2, CheckCircle2, AlertCircle, Clock, BookOpen, BarChart2, Hash, FileText, Calendar, RefreshCw, Presentation, X, Loader2, Play, Download, Sparkles } from 'lucide-react';
...
export default function AdminEditTest() {
  ...
  const handleExportPPT = async (isTeachingMode) => {
    ...
    try {
      ...
      await exportTestToPPTX(test, questions, { isTeachingMode });
      toast.success('PowerPoint slideshow downloaded successfully!', { id: toastId });
      setIsPPTModalOpen(false);
    } catch (error) {
      console.error('Error generating PPT:', error);
      toast.error('Failed to generate PowerPoint slides.', { id: toastId });
    } finally {
  ...
  const handleLaunchWebPresentation = () => {
    window.open(`/admin/tests/${testId}/present`, '_blank');
    setIsPPTModalOpen(false);
  };
    ...
    <div className="space-y-8 pb-20">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
        <div>
          <button 
            ...
            className="flex items-center gap-2 text-slate-400 hover:text-slate-900 transition-colors text-[10px] font-bold uppercase tracking-widest mb-4"
          >
            <ArrowLeft className="w-4 h-4" /> Back to Tests
          </button>
          <h2 className="text-3xl font-bold text-slate-900 tracking-tight">Edit Assessment</h2>
        ...
        <div className="flex gap-4">
          ...
          <button
            ...
            className="flex items-center gap-2 px-6 py-3 bg-blue-50 hover:bg-blue-100 border border-blue-100 text-blue-600 rounded-2xl font-bold uppercase tracking-widest transition-all shadow-sm active:scale-95"
          >
          ...
          <button
            ...
            disabled={isSaving}
            className="flex items-center gap-2 px-6 py-3 bg-primary-600 hover:bg-primary-700 text-white rounded-2xl font-bold uppercase tracking-widest transition-all shadow-lg shadow-primary-500/10 active:scale-95 disabled:opacity-50"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-white border border-slate-100 rounded-3xl shadow-sm p-8 relative overflow-hidden group">
            <div className="absolute top-0 right-0 w-32 h-32 bg-primary-500/5 rounded-full blur-3xl -mr-16 -mt-16 transition-transform group-hover:scale-150 duration-700"></div>
            
            <h3 className="text-sm font-bold text-slate-900 uppercase tracking-widest flex items-center gap-3 mb-8">
              <span className="w-8 h-8 rounded-lg bg-primary-50 flex items-center justify-center text-primary-600 shadow-sm border border-primary-100">
            ...
            <form onSubmit={handleSaveTest} className="space-y-6">
              ...
              <div className="space-y-3">
                ...
                <input
                  ...
                  className="w-full px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl text-sm text-slate-900 focus:ring-2 focus:ring-primary-500/20 transition-all font-bold placeholder:text-slate-400"
                />
              </div>
                ...
                <div className="grid grid-cols-2 gap-4 border-t border-slate-100 pt-6 mt-2">
                  <div className="space-y-3">
                    ...
                    <input
                      ...
                      className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-900 focus:ring-2 focus:ring-primary-500/20 transition-all font-bold"
                    />
                  </div>
                  <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-3">
                   <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2"><Clock className="w-3 h-3 text-primary-600"/> Duration</label>
                   <input
                    name="duration" required value={test.duration} onChange={handleTestChange}
                    className="w-full px-5 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-900 focus:ring-2 focus:ring-primary-500/20 transition-all font-bold placeholder:text-slate-400"
                  />
                </div>
                <div className="space-y-3">
                   <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2"><BarChart2 className="w-3 h-3 text-primary-600"/> Marks</label>
                   <input
              ...
              <div className="pt-4">
                <div className="flex items-center justify-between p-4 rounded-2xl bg-slate-50 border border-slate-100 shadow-sm">
                   <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl bg-primary-100 text-primary-600 flex items-center justify-center border border-primary-200">
                        <Hash className="w-5 h-5" />
                      </div>
                      <div>
                        <p className="text-[10px] uppercase tracking-widest font-bold text-slate-400">Total Questions</p>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white border border-slate-100 rounded-3xl shadow-sm p-8 min-h-[600px] flex flex-col">
            <div className="flex justify-between items-center mb-8 pb-6 border-b border-slate-100">
              <h3 className="text-sm font-bold text-slate-900 uppercase tracking-widest flex items-center gap-3">
                <span className="w-8 h-8 rounded-lg bg-emerald-50 flex items-center justify-center text-emerald-600 shadow-sm border border-emerald-100">
            ...
            {questions.length === 0 ? (
              <div className="flex-1 flex flex-col items-center justify-center text-center opacity-50">
                ...
                <p className="text-lg font-bold text-slate-400 mb-2">No Questions Found</p>
              ...
              <div className="flex-1 overflow-y-auto pr-2 space-y-4 custom-scrollbar">
                {questions.map((q, index) => (
                  <div key={q.id} className="p-6 rounded-2xl bg-slate-50 border border-slate-100 hover:border-primary-100 transition-all group relative shadow-sm">
                    <div className="flex justify-between items-start gap-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-3">
                          <span className="px-2.5 py-1 rounded bg-white text-[10px] font-bold text-slate-400 tracking-widest border border-slate-100 shadow-sm">Q{index + 1}</span>
                          <span className="px-2.5 py-1 rounded bg-amber-50 text-[10px] font-bold text-amber-600 tracking-widest border border-amber-100 shadow-sm">{q.marks} Marks</span>
                        </div>
                        <h4 className="text-base font-bold text-slate-900 leading-relaxed mb-4">{q.question}</h4>
                        
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          {['A', 'B', 'C', 'D'].map(optKey => {
                            const isCorrect = q.correctAnswer === optKey;
                            return (
                              <div key={optKey} className={`p-3 rounded-xl border flex gap-3 text-sm shadow-sm transition-all ${
                                isCorrect 
                                  ? 'border-emerald-200 bg-emerald-50 text-emerald-900' 
                                  : 'border-slate-100 bg-white text-slate-500'
                              }`}>
                                <span className={`font-bold ${isCorrect ? 'text-emerald-600' : 'text-slate-400'}`}>{optKey}.</span>
      ...
      {isQuestionModalOpen && editingQuestion && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
           <div className="bg-white border border-slate-200 rounded-3xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl relative animate-in zoom-in-95 duration-300 flex flex-col">
              <div className="sticky top-0 bg-white border-b border-slate-100 px-8 py-6 flex items-center justify-between z-10">
                ...
                <button onClick={() => setIsQuestionModalOpen(false)} className="text-slate-400 hover:text-slate-900 transition-colors">
              ...
              <form onSubmit={handleSaveQuestion} className="p-8 space-y-6">
                ...
                <div className="space-y-3">
                  ...
                  <textarea
                    ...
                    className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm text-slate-900 focus:ring-2 focus:ring-primary-500/20 transition-all resize-none font-bold placeholder:text-slate-400"
                    ...
                    <div key={optKey} className="space-y-3">
                      <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest flex justify-between">
                        ...
                        {editingQuestion.correctAnswer === optKey && (
                          <span className="text-emerald-600 flex items-center gap-1 font-bold"><CheckCircle2 className="w-3 h-3"/> Correct</span>
                        )}
                      </label>
                      <input
                        required
                        ...
                        className={`w-full px-4 py-3 bg-slate-50 border-2 rounded-xl text-sm transition-all font-bold ${
                          editingQuestion.correctAnswer === optKey ? 'border-emerald-200 text-emerald-900' : 'border-slate-100 text-slate-600 focus:border-slate-200'
                        }`}
                ...
                <div className="grid grid-cols-2 gap-4 border-t border-slate-100 pt-6 mt-4">
                   <div className="space-y-3">
                      <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Correct Answer</label>
                      <select
                        value={editingQuestion.correctAnswer}
                        onChange={(e) => setEditingQuestion({...editingQuestion, correctAnswer: e.target.value})}
                        className="w-full px-5 py-3.5 bg-emerald-50 border border-emerald-200 rounded-2xl text-sm text-emerald-700 font-bold focus:ring-2 focus:ring-emerald-500/20 transition-all cursor-pointer appearance-none shadow-sm"
                      >
                   ...
                   <div className="space-y-3">
                      ...
                      <input
                        ...
                        className="w-full px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-2xl text-sm text-slate-900 focus:ring-2 focus:ring-primary-500/20 transition-all font-bold shadow-sm"
                      />
                   </div>
                </div>
                ...
                <div className="flex gap-4 pt-4 border-t border-slate-100">
                  <button
                    ...
                    className="flex-1 py-4 px-4 bg-slate-50 hover:bg-slate-100 text-slate-900 border border-slate-200 rounded-2xl font-bold text-[10px] uppercase tracking-widest transition-all active:scale-95 shadow-sm"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    className="flex-1 py-4 px-4 bg-primary-600 hover:bg-primary-700 text-white rounded-2xl font-bold text-[10px] uppercase tracking-widest shadow-lg shadow-primary-500/10 transition-all active:scale-95"
                  >
                    Save Question
                  </button>
                </div>
      ...
      {isRescheduleOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white border border-slate-200 rounded-3xl w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-300">
            <div className="px-8 pt-8 pb-6 border-b border-slate-100 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-bold text-slate-900 uppercase tracking-widest">Reschedule Test</h3>
                  <p className="text-[10px] text-slate-500 font-bold mt-0.5">Status will reset to Upcoming</p>
                </div>
              </div>
              <button onClick={() => setIsRescheduleOpen(false)} className="text-slate-400 hover:text-slate-900">
            ...
            <form onSubmit={handleReschedule} className="p-8 space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-3">
                  <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2">
                    <Calendar className="w-3 h-3 text-primary-600" /> New Date
                  </label>
                  <input
                    type="date" required
                    value={rescheduleDate}
                    onChange={e => setRescheduleDate(e.target.value)}
                    className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-900 focus:ring-2 focus:ring-blue-500/20 transition-all font-bold shadow-sm"
                  />
                </div>
                <div className="space-y-3">
                  <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2">
                    <Clock className="w-3 h-3 text-primary-600" /> New Time
                  </label>
                  <input
                    type="time" required
              ...
              <div className="p-4 bg-blue-50 border border-blue-100 rounded-2xl shadow-sm">
                <p className="text-[10px] font-bold text-blue-600 uppercase tracking-widest">ℹ️ Auto-launch will publish this test at the new scheduled time.</p>
              </div>

              <div className="flex gap-4">
                <button
                  type="button" onClick={() => setIsRescheduleOpen(false)}
                  className="flex-1 py-4 bg-slate-50 hover:bg-slate-100 text-slate-900 border border-slate-200 rounded-2xl font-bold text-[10px] uppercase tracking-widest transition-all active:scale-95 shadow-sm"
                >
                  Cancel
                </button>
                <button
                  type="submit" disabled={isRescheduling}
                  className="flex-1 py-4 bg-blue-600 hover:bg-blue-700 text-white rounded-2xl font-bold text-[10px] uppercase tracking-widest shadow-lg shadow-blue-500/10 transition-all active:scale-95 disabled:opacity-50"
                >
          ...
          <div className="bg-white border border-slate-150 rounded-3xl w-full max-w-lg shadow-2xl animate-in zoom-in-95 duration-300 overflow-hidden">
            ...
            <div className="p-8 space-y-4">
              ...
              <button
                ...
                className="w-full p-4 rounded-2xl border border-slate-100 bg-slate-50/30 hover:bg-slate-50 hover:border-slate-200 text-left flex items-start gap-4 transition-all group active:scale-[0.98] disabled:opacity-50"
              >
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[8px] font-bold text-blue-600 bg-blue-50 border border-blue-100 px-1.5 py-0.5 rounded uppercase tracking-wider font-extrabold">Default</span>
                  </div>
                  <span className="block text-[10px] text-slate-400 font-medium mt-1">Generates standard slides. Answers are grouped into a compact answer key slide at the end.</span>
                </div>
              </button>
              ...
              >
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[8px] font-bold text-emerald-600 bg-emerald-50 border border-emerald-100 px-1.5 py-0.5 rounded uppercase tracking-wider font-extrabold">Answers Highlighted</span>
                  </div>
                  <span className="block text-[10px] text-slate-400 font-medium mt-1">Highlights the correct answer on every question slide in emerald green. Ideal for study sheets or screen shares.</span>
                </div>
              </button>
              ...
              <button
                ...
                className="w-full p-4 rounded-2xl border border-indigo-100 bg-indigo-50/20 hover:bg-indigo-50 hover:border-indigo-200 text-left flex items-start gap-4 transition-all group active:scale-[0.98]"
              >
                ...
                <div className="flex-1">
                  ...
                  <span className="block text-[10px] text-slate-400 font-medium mt-1">Open interactive browser slide presenter with dynamic answer reveal shortcuts and custom poll timers.</span>
                </div>
              </button>
            </div>
