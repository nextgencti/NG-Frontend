...
export default function SuperAdminControls() {
  ...
  const handleOpenEditModal = (service) => {
    ...
    setServiceForm({
      ...
      hasDetailsPage: service.hasDetailsPage || false,
      ageLimit: service.ageLimit || '',
      feesDetails: service.feesDetails || '',
      importantDates: service.importantDates || '',
      requiredDocs: service.requiredDocs ? service.requiredDocs.join(', ') : '',
      formFillGuide: service.formFillGuide || '',
  ...
  const handleSaveService = async (e) => {
    ...
    try {
      ...
      formData.append('hasDetailsPage', serviceForm.hasDetailsPage);
      formData.append('ageLimit', serviceForm.ageLimit);
      formData.append('feesDetails', serviceForm.feesDetails);
      formData.append('importantDates', serviceForm.importantDates);
      
      const docsArray = serviceForm.requiredDocs 
        ? serviceForm.requiredDocs.split(',').map(d => d.trim()).filter(Boolean)
        : [];
      formData.append('requiredDocs', JSON.stringify(docsArray));
      formData.append('formFillGuide', serviceForm.formFillGuide);
      formData.append('youtubeUrl', serviceForm.youtubeUrl);
      formData.append('sublinks', JSON.stringify(serviceForm.sublinks));
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl border border-slate-200 shadow-2xl w-full max-w-xl max-h-[90vh] overflow-y-auto animate-in zoom-in-95 duration-200 text-left">
            <div className="px-6 py-4.5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50 sticky top-0 z-10 backdrop-blur-md">
            ...
            <form onSubmit={handleSaveService} className="p-6 space-y-5">
              ...
              {/* Has Details Page checkbox toggle */}
              <div className="flex items-center gap-3 p-3 bg-indigo-50/40 rounded-2xl border border-indigo-100/40 animate-in fade-in duration-300">
                <input
                  type="checkbox"
                  id="has-details-page"
                  checked={serviceForm.hasDetailsPage}
                  onChange={(e) => setServiceForm(prev => ({ ...prev, hasDetailsPage: e.target.checked }))}
                  className="w-4.5 h-4.5 text-indigo-650 bg-slate-100 border-slate-300 rounded focus:ring-indigo-500 cursor-pointer"
                />
                <label htmlFor="has-details-page" className="text-xs font-bold text-slate-700 cursor-pointer select-none">
                  Enable Detailed Information Page for this Service
                </label>
              </div>

              {/* Rich Details Conditional Panel */}
              {serviceForm.hasDetailsPage && (
                <div className="border border-indigo-150 rounded-2xl p-4 bg-indigo-50/15 space-y-4 animate-in slide-in-from-top-3 duration-300">
                  <h4 className="text-[11px] font-black uppercase tracking-wider text-indigo-650 mb-3 border-b border-indigo-100 pb-1.5">
                    Service Detailed Information
                  </h4>

                  {/* 3-Column: Dates, Age, Fees */}
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    <div className="space-y-1">
                      <label className="text-[9px] font-black text-slate-500 uppercase tracking-wider">Important Dates</label>
                      <input
                        type="text"
                        value={serviceForm.importantDates}
                        onChange={(e) => setServiceForm(prev => ({ ...prev, importantDates: e.target.value }))}
                        placeholder="e.g. Apply Start: 01-07"
                        className="w-full px-2.5 py-2 text-xs bg-white border border-slate-200 rounded-lg outline-none focus:ring-1 focus:ring-indigo-500"
                      />
                    </div>
                    <div className="space-y-1">
                      <label className="text-[9px] font-black text-slate-500 uppercase tracking-wider">Age Limit</label>
                      <input
                        type="text"
                        value={serviceForm.ageLimit}
                        onChange={(e) => setServiceForm(prev => ({ ...prev, ageLimit: e.target.value }))}
                        placeholder="e.g. Min: 18, Max: 27"
                        className="w-full px-2.5 py-2 text-xs bg-white border border-slate-200 rounded-lg outline-none focus:ring-1 focus:ring-indigo-500"
                      />
                    </div>
                    <div className="space-y-1">
                      <label className="text-[9px] font-black text-slate-500 uppercase tracking-wider">Fees Details</label>
                      <input
                        type="text"
                        value={serviceForm.feesDetails}
                        onChange={(e) => setServiceForm(prev => ({ ...prev, feesDetails: e.target.value }))}
                        placeholder="e.g. Gen: ₹100"
                        className="w-full px-2.5 py-2 text-xs bg-white border border-slate-200 rounded-lg outline-none focus:ring-1 focus:ring-indigo-500"
                      />
                    </div>
                  </div>
                  ...
                  <div className="space-y-1">
                    <label className="text-[9px] font-black text-slate-500 uppercase tracking-wider">Required Documents (Comma-separated)</label>
                    <textarea
                      value={serviceForm.requiredDocs}
                      onChange={(e) => setServiceForm(prev => ({ ...prev, requiredDocs: e.target.value }))}
                      placeholder="e.g. Aadhaar Card, Passport Photo, Class 10 Marksheet"
                      rows="2"
                      className="w-full px-2.5 py-2 text-xs bg-white border border-slate-200 rounded-lg outline-none focus:ring-1 focus:ring-indigo-500 resize-none"
                    />
                    <p className="text-[8.5px] text-slate-400">Add documents separated by commas. Each will show as a bullet checklist.</p>
                  </div>

                  {/* YouTube Tutorial URL */}
                  <div className="space-y-1">
                    <label className="text-[9px] font-black text-slate-500 uppercase tracking-wider">YouTube Tutorial Video URL</label>
                    <input
                      type="url"
                      value={serviceForm.youtubeUrl}
                      onChange={(e) => setServiceForm(prev => ({ ...prev, youtubeUrl: e.target.value }))}
                  ...
                  <div className="space-y-1">
                    <label className="text-[9px] font-black text-slate-500 uppercase tracking-wider">Step-by-step Form Filling Instructions</label>
                    <textarea
                      value={serviceForm.formFillGuide}
                      onChange={(e) => setServiceForm(prev => ({ ...prev, formFillGuide: e.target.value }))}
                      placeholder="Enter instructions (Supports Markdown, e.g. **Step 1**: Register...)"
                      rows="4"
                      className="w-full px-2.5 py-2 text-xs bg-white border border-slate-200 rounded-lg outline-none focus:ring-1 focus:ring-indigo-500 resize-y"
                    />
                  </div>

                  {/* Dynamic Sublinks array list */}
                  <div className="space-y-2.5 border-t border-indigo-100 pt-3">
                    <div className="flex items-center justify-between">
                      <label className="text-[9px] font-black text-slate-500 uppercase tracking-wider">Custom Sublinks</label>
                      <button
                        type="button"
                        onClick={() => setServiceForm(prev => ({ 
                          ...prev, 
                          sublinks: [...prev.sublinks, { label: '', url: '' }] 
                        }))}
                        className="inline-flex items-center gap-1 text-[9px] font-black uppercase text-indigo-650 hover:text-indigo-800 transition-colors cursor-pointer"
                      >
                    ...
                    <div className="space-y-2">
                      {serviceForm.sublinks.map((sub, idx) => (
                        <div key={idx} className="flex items-center gap-2 bg-white p-2 rounded-lg border border-slate-100 shadow-sm">
                          <input
                            type="text"
                            value={sub.label}
                            onChange={(e) => {
                              const updated = [...serviceForm.sublinks];
                              updated[idx].label = e.target.value;
                              setServiceForm(prev => ({ ...prev, sublinks: updated }));
                            }}
                            placeholder="e.g. Apply Link"
                            required
                            className="w-1/3 px-2 py-1 text-[11px] bg-slate-50 border border-slate-200 rounded outline-none focus:ring-1 focus:ring-indigo-500"
                          />
                          <input
                            type="url"
                            value={sub.url}
                            ...
                            required
                            className="flex-1 px-2 py-1 text-[11px] bg-slate-50 border border-slate-200 rounded outline-none focus:ring-1 focus:ring-indigo-500"
                          />
                          <button
                            type="button"
                            onClick={() => {
                              const updated = serviceForm.sublinks.filter((_, subIdx) => subIdx !== idx);
                              setServiceForm(prev => ({ ...prev, sublinks: updated }));
                            }}
                      ...
                      {serviceForm.sublinks.length === 0 && (
                        <p className="text-[9px] text-slate-400 italic text-center py-1">No custom sublinks added yet.</p>
                      )}
                    </div>
                  </div>
                </div>
